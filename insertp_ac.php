...
mysql_select_db("user_db")or die("cannot select DB");
    ...
    isset( $_FILES['p_img']['tmp_name'] ) ? $image_tmp_name = $_FILES['p_img']['tmp_name'] : $image_tmp_name = "";
    isset( $_FILES['p_img']['name'] ) ? $p_img = $_FILES['p_img']['name'] : $p_img = "";
    if( !empty( $image_tmp_name ) && !empty( $p_img ) ) {
        if( move_uploaded_file( $image_tmp_name, $p_img ) ) {
    $p_num=$_POST['p_num'];
    
    // Insert data into mysql 
    $sql="INSERT INTO product VALUES('$p_id', '$p_name', '$p_detail', '$p_cost', '$p_price', '$p_img', '$p_num' , '$ct_id' )";
...
?> 
<br>
<a href="listp_records.php"><input type="button" name="record" value="กลับหน้ารายการสินค้า"></a>
