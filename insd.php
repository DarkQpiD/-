...
$o_id=$_GET['o_id'];
include('connect.php');
$sql = "SELECT pay_id FROM payb
        INNER JOIN order_head ON payb.o_id = order_head.o_id
        WHERE payb.o_id = '$o_id'";
		$query	= mysqli_query($conn, $sql);
		$row	= mysqli_fetch_array($query);
?>
<form name="form1" method="post" action="inds_ac.php" enctype="multipart/form-data">
<center><br>
<h3>รายละเอียดการจัดส่งของหมายเลขออเดอร์ <?php echo $o_id; ?></h3><br>
<table>
<tr>
<td width="150">หมายเลขชำระเงิน</td>
<td width="10">:</td>
<td width="301"><input name="pay_id" type="hidden" id="pay_id" value="<?php echo $row['pay_id']; ?>"><?php echo $row['pay_id']; ?></td>
</tr>
<tr>
    <td>หมายเลขพัสดุ</td>
    <td>:</td>
    <td><input name="d_id" type="text" id="d_id"></td>
</tr>
<tr>
    <td>วันที่ส่งพัสดุ</td>
    <td>:</td>
    <td><input name="d_date" type="date" id="d_date"></td>
</tr>
<tr>
    <td>บริษัทขนส่ง</td>
    <td>:</td>
...
<br>
<p><input type="submit" name="Submit" value="ตกลง"></p>
</form>
<a href="report3.php"><input type="button" name="record" value="กลับ"></a>
</center>
<!-- Footer Start -->
