...
<center><br>
<h3>รายงานข้อมูลยอดขายที่ขายได้</h3><br>
<form method="GET" id="form" enctype="multipart/form-data" action=" ">
    <font style='font-size:18px'>ค้นหาข้อมูล
    <input type="text" name="search" size="20" value="">
    <input type="submit" value="ตกลง">
</form>
<p> </p>

<table width="30%" cellspacing="0" cellpadding="9">
    <tr>
        <td align="center"><font size = "4">วันที่</td>
        <td align="center"><font size = "4">ราคา</td>
    </tr>

    <?php
        $search=isset($_GET['search']) ? $_GET['search']:'';
        $total = 0;
        $sql ="SELECT o_qty,pay_date, SUM(o_sum) as pTotal FROM order_detail
        INNER JOIN order_head
        ON order_detail.o_id = order_head.o_id
        INNER JOIN payb
        ON order_head.o_id = payb.o_id
        WHERE (order_head.s_id='DY' OR (order_head.s_id='PY')) AND (pay_date LIKE '%$search')
        GROUP BY pay_Date";
        ...
        <tr>
            <td align="center"><font style='font-size:18px'><?= date("d-m-Y",strtotime($row['pay_date']));?></td>
            <td align="center"><font style='font-size:18px'><?= $row['pTotal'];?></td>
        </tr>

<?php } ?>
        
        <tr>
            <td align="center"><font style='font-size:18px'>รวมทั้งหมด</td>
            <td align="center"><font style='font-size:18px'><?=$total;?></td>
        </tr> 
